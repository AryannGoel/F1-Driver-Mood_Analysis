...
def _gp_rows(dataset_gp, max_rows=600):
    ...
    GP is only paged from the slow HF /filter a single time."""
...
def fetch_clip_rows(dataset_gp, racing_number, limit=40):
    ...
    include the year exactly as stored, e.g. '2020 Turkish Grand Prix'."""
...
def import_driver_radio(year, gp_name, session, driver,
    ...
    # pick one clip per lap (earliest) inside the window
    picks = {}   # lap -> (url, when, transcription)
    for row in rows:
        lap = _clip_to_lap(row["message_timestamp"], lap_starts)
        if lap is None or not (lap_from <= lap <= lap_to) or lap in picks:
            continue
        picks[lap] = (row["audio"][0]["src"], row["message_timestamp"],
                      row.get("transcription"))

    def _download(lap, url):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as pool:
        futures = {pool.submit(_download, lap, url): (lap, when, tr)
                   for lap, (url, when, tr) in picks.items()}
